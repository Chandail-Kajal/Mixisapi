import React from "react";

const Keyword = () => {
  return (
    <div className="flex flex-col min-h-screen w-full max-w-[100vw] overflow-x-hidden">
      <div className="flex-grow">
        {/* Hero Section */}
        <div className="w-full bg-green-50 py-2">
          <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div>
              <div className="flex flex-wrap gap-6 mb-5">
                <div className="flex items-center gap-2 rounded-full px-4 py-1 text-xs sm:text-base text-black font-semibold bg-green-200 backdrop-blur-sm shadow-sm hover:border-green-500">
                  ⚡Keyword-Based Chatbot Builder
                </div>
              </div>
              <h2 className="text-4xl font-extrabold text-black">
                Keyword-Based Chatbot Builder
              </h2>
              <h4 className="text-xl font-bold text-gray-500">User-Friendly Setup</h4>
              <p className="text-sm text-gray-500 mb-4">
                Quickly get started by assigning keywords and predefined replies.
              </p>
              <h4 className="text-xl font-bold text-gray-500">Budget-Friendly Option</h4>
              <p className="text-sm text-gray-500 mb-4">
                Perfect for small businesses or startups on a tight budget.
              </p>
              <h4 className="text-xl font-bold text-gray-500">Complete Control Over Responses</h4>
              <p className="text-sm text-gray-500 mb-4">
                Specify exactly what your bot will say for each keyword trigger.
              </p>
              <h4 className="text-xl font-bold text-gray-500">Immediate Replies</h4>
              <p className="text-sm text-gray-500 mb-4">
                Provides quick, precise responses without any lag.
              </p>
              <h4 className="text-xl font-bold text-gray-500">Ideal for FAQs & Lead Generation</h4>
              <p className="text-sm text-gray-500 mb-4">
                Excellent for managing straightforward inquiries and gathering customer data.
              </p>
              <p className="text-sm text-gray-500 flex items-center gap-1">
                <span className="text-yellow-600">▢</span> No credit card required
              </p>
              <div className="mt-3">
                <img
                 src="data:image/webp;base64,"
                
                  alt="Meta Business Partner"
                  className="h-14"
                />
              </div>
            </div>
            <div className="flex justify-center">
              <img src="https://mittosapi.com/assets/keyword-C50Dp62v.png" alt="Chatbot illustration" />
            </div>
          </div>
        </div>

        

       
      </div>
    </div>
  );
};

export default Keyword;
