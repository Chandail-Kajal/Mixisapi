import React from "react";

function Widgets() {
  return (
    <>
      <div className="w-full bg-green-50 py-2 ">
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div>
            <div className="flex flex-wrap gap-6 mb-5">
              <div className="flex items-center gap-2 rounded-full px-4 py-1 text-xs sm:text-base text-black font-semibold bg-green-200 backdrop-blur-sm shadow-sm hover:border-green-500">
                ⚡Add Live Chat Icon On Your Website &amp; App
              </div>
            </div>
            <h2 className="text-4xl font-extrabold text-black">
              WhatsApp Chat Widget
            </h2>
            <h4 className="text-xl font-bold text-gray-500">
              Benifits of Using a WhatsApp Chat Widget
            </h4>
            <p className="text-sm text-gray-500 mb-4">
              WhatsApp chat buttons can pique the interest of high-intent leads
              browsing your website and nudge them to explore your offerings
              over a conversations.Leave less room for indecision from your
              potential buyers by bringning them to WhatsApp and move the needle
              faster through your lead lifecycle.
            </p>
            <h4 className="text-xl font-bold text-gray-500">
              Make reluctant prospect more conversation ready
            </h4>
            <p className="text-sm text-gray-500 mb-4">
              Direct website traffic to your WhatsApp Business account, where
              your sales folks can have more control over the prospect's buying
              journey.
            </p>
            <h4 className="text-xl font-bold text-gray-500">
              Qualify leads before getting into a live conversation
            </h4>
            <p className="text-sm text-gray-500 mb-4">
              Boost your sales and marketing team's productivity by setting up
              lead qualification Workflows within the WhatsApp chat widget.
            </p>
            <h4 className="text-xl font-bold text-gray-500">
              Shorten sales cycles with AI-assisted buying
            </h4>
            <p className="text-sm text-gray-500 mb-4">
              Accelerate your time to move between lead creation and deal
              closure with an intuitive mix of WhatsApp automation and human
              interaction, powered by Gen AI.
            </p>
            <div className="flex items-center gap-3 mb-3">
              <a href="/forms" data-discover="true">
                <button className="bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-2 rounded-lg shadow">
                  Start FREE Trial
                </button>
              </a>
              <a href="/pricing" data-discover="true">
                <button className="bg-red-600 hover:bg-red-700 text-white font-semibold px-5 py-2 rounded-lg shadow flex items-center gap-1">
                  ▶ Check Pricing
                </button>
              </a>
            </div>
            <p className="text-sm text-gray-500 flex items-center gap-1">
              <span className="text-yellow-600">▢</span> No credit card required
            </p>
            <div className="mt-3">
              <img
                src="data:image/webp;base64,"
                alt="Meta Business Partner"
                className="h-14"
              />
            </div>
          </div>
          <div className="flex justify-center">
            <img
              src="https://mittosapi.com/assets/Sequence-CnV-xWNj.png"
              alt="WhatsApp Demo"
              className="max-w-md w-full h-[550px] "
            />
          </div>
        </div>
      </div>
      <div className="bg-green-50 flex items-center justify-center px-10 py-5">
        <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div>
            <div className="flex flex-wrap gap-6 mb-5">
              <div className="flex items-center gap-2 rounded-full px-4 py-1 text-xs sm:text-base text-black font-semibold bg-green-200 backdrop-blur-sm shadow-sm hover:border-green-500">
                ⚡ CTWA: Click To WhatsApp Ads
              </div>
            </div>
            <h1 className="text-4xl font-bold mb-4 leading-snug">
              <span className="text-green-600">Click To </span>
              <span className="text-black">WhatsApp Ads ( CTWA )</span>
            </h1>
            <p className="text-gray-700 mb-6">
              Never miss leads, even without numbers! Engage customers directly
              on WhatsApp, nurture leads affordably, sell in real time, and
              shorten your sales funnel effortlessly with CTWA WhatsApp
              marketing.
            </p>
            <div className="flex gap-4 mb-3">
              <a href="/forms" data-discover="true">
                <button className="bg-green-700 text-white px-5 py-3 rounded-md font-medium hover:bg-green-800 transition">
                  Start FREE Trial
                </button>
              </a>
              <a href="/pricing" data-discover="true">
                <button className="bg-red-600 text-white px-5 py-3 rounded-md font-medium hover:bg-red-700 transition">
                  Check Pricing
                </button>
              </a>
            </div>
            <p className="text-sm text-gray-500">🟨 No credit card required</p>
            <div className="mt-3">
              <img
                src="data:image/webp;base64,"
                alt="Meta Business Partner"
                className="h-14"
              />
            </div>
          </div>
          <div className="flex justify-center">
            <img
              src="https://mittosapi.com/assets/adimg-BSgCHHJm.png"
              alt="CTWA WhatsApp Ad Mockup"
              className="max-w-full rounded-xl shadow-lg"
            />
          </div>
        </div>
      </div>
    </>
  );
}

export default Widgets;
